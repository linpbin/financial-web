...
<script>
    import {incomeType, updateincomeType, userIncomeType, deleteincomeType, postincomeType} from '../../api/api';

    export default {
        data() {
            return {
                tableData3: JSON.parse(sessionStorage.getItem('userincometype')),
                multipleSelection: [],
                form: {
                    typeName: '',
                },
        ...
        methods: {
            ...
            handleEdit(row) {
                this.$prompt('请输入类型名称', '修改收入类型', {
                    ...
                    console.log(params)
                    updateincomeType(id, params).then(data => {
                        if (data.resultCode == 0) {
                            userIncomeType(row.userId).then(data => {
            ...
            handleDelete(row) {
                deleteincomeType(row.id).then(data => {
                    if (data.resultCode == 0) {
                        userIncomeType(row.userId).then(data => {
            ...
            addType(){
                this.$prompt('请输入类型名称', '添加收入类型', {
                    ...
                    postincomeType(this.form).then(data => {
                        if (data.resultCode == 0){
                            userIncomeType(userid.id).then(data => {
                                let incometype = JSON.stringify(data.data);
                                sessionStorage.setItem("userincometype", incometype);
                                this.tableData3 = data.data;
                            })
            ...
            multiDelete(){
                ...
                for (let id in ids){
                    deleteincomeType(ids[id]).then(data =>{
                        if (data.resultCode == 0) {
                            var userid = sessionStorage.getItem('access-token');
                            userid = JSON.parse(userid);
                            userIncomeType(userid.id).then(data => {
