        ...
        <el-row>
            <el-col :span="24">
                <el-button-group>
                    ...
                    <el-dialog title="添加收入记录" :visible.sync="addincomes">
                        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px"
                                 class="demo-ruleForm">
                            <el-form-item label="收入金额" prop="sum">
                                <el-input v-model.number="ruleForm.sum" type="sum"></el-input>
                            </el-form-item>
                            <el-form-item label="收入类型" prop="expendType">
                                <el-select v-model="ruleForm.incomeType" placeholder="请选择" prop>
                                    <el-option
                            ...
                            <el-form-item label="收入时间" required prop="incomeDate">
                                <div class="block">
                                    <el-date-picker
                                            v-model="ruleForm.incomeDate"
                                            type="datetime"
                                            placeholder="选择日期时间"
                                            prop="date">
                                    </el-date-picker>
...
<script>
    import {income, incomeType, postincomeType,incomelist,updateincome,deleteincome} from '../../api/api';
    ...
    export default {
        ...
        data() {
            return {
                tableData3: (JSON.parse(sessionStorage.getItem('incomelist'))).list,
                pageSize: '10',
                pageNo: '1',
                total: (JSON.parse(sessionStorage.getItem('incomelist'))).total,
                multipleSelection: [],
                addincomes: false,
                options: JSON.parse(sessionStorage.getItem('incometype')),
                ruleForm: {
                    sum: '',
                    incomeType: '',
                    remark: '',
        ...
        methods: {
            getPageSize(val) {
                ...
                incomelist(`${userid.id}&pageNo=${this.pageNo}&pageSize=${this.pageSize}`).then(data => {
                    if (data.data != null) {
                        var arr = data.data.list;
                        for (const index in arr){
                            arr[index].time= this.formatDate(arr[index].incomeDate)
                        }
                        this.tableData3 = arr
            ...
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        income(this.ruleForm).then(data => {
            ...
            addType() {
                this.$prompt('请输入类型名称', '添加收入类型', {
                    ...
                    postincomeType(this.form).then(data => {
                        if (data.resultCode == 0) {
                            incomeType(userid.id).then(data => {
