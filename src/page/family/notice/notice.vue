        ...
        <el-row>
            <el-col :span="24">
                <template>
                    ...
                    <el-dialog title="发布公告" :visible.sync="postnotice">
                ...
                <el-table
                    ...
                    <el-table-column label="操作">
                        <template slot-scope="scope">
                            ...
                            <el-dialog title="修改公告内容" :visible.sync="updatenotice">
...
<script>
    import {familyuser, getnotices, updatenotices, createnotices, deletenotices} from '../../../api/api';
    ...
    export default {
        data() {
            return {
                ...
                pageSize: '10',
                pageNo: '1',
                total: (JSON.parse(sessionStorage.getItem('notices'))).total,
        ...
        methods: {
            getPageSize(val) {
                this.pageSize = val;
                getnotices(`${this.familyinfo.id}?pageNo=${this.pageNo}&pageSize=${this.pageSize}`).then(data => {
...
//                  if (valid){
                      updatenotices(this.familyinfo.id, this.userId.id, this.editId, this.editform).then(data => {
                          if (data.resultCode == 0) {
                              getnotices(this.familyinfo.id).then(data => {
                                  var arr = data.data.list
...
//              })
            ...
            handleDelete(row) {
                deletenotices(this.familyinfo.id, this.userId.id, row.id).then(data => {
                    if (data.resultCode == 0) {
                        getnotices(this.familyinfo.id).then(data => {
                            var arr = data.data.list
                            for (const index in arr) {
            ...
            addNotice(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        createnotices(this.familyinfo.id, this.userId.id, this.createform).then(data => {
                            if (data.resultCode == 0) {
                                getnotices(this.familyinfo.id).then(data => {
                                    var arr = data.data.list
                                    this.total=data.data.total
